...
class AxisMembersModel:
    ...
    # customize_hover_data: bool = field(default=False, metadata={"label": "Customize Hover Data:", "widget": QCheckBox, "plot_types": ["ternary", "cartesian"]})
    # hover_data: list = field(default_factory=list, metadata={"label": "Hover Data:", "widget": MultiFieldSelector, "plot_types": ["ternary", "cartesian"], "depends_on": "customize_hover_data"})
...
class SetupMenuModel:
    ...
    def __getattr__(self, name):
        # Delegate lookup to sub-models if the attribute isn't found on self.
        for submodel in [self.plot_labels, self.axis_members, self.advanced_settings]:
            if hasattr(submodel, name):
                return getattr(submodel, name)
...
class SetupMenuController:
    ...
    def update_axis_options(self):
        ...
        # TODO handle case with no common columns
...
class SetupMenuView(QWidget):
    def __init__(self, model: SetupMenuModel, parent=None):
        ...
        # --- Top-level toggle checkboxes ---
        self.topToggleWidget = QWidget(self)
        toggle_layout = QHBoxLayout(self.topToggleWidget)
        self.hoverDataToggle = QCheckBox("Customize Hover Data", self.topToggleWidget)
        self.advancedSettingsToggle = QCheckBox("Use Advanced Settings", self.topToggleWidget)
        toggle_layout.addWidget(self.hoverDataToggle)
        toggle_layout.addWidget(self.advancedSettingsToggle)
        toggle_layout.addStretch()
        self.content_layout.addWidget(self.topToggleWidget)
        
        # --- Hover Data GroupBox ---
        self.hoverDataGroupBox = QGroupBox("Hover Data Settings", self)
        hover_layout = QFormLayout(self.hoverDataGroupBox)
        # Create the hover data selection widget.
        # (If necessary, move the hover data logic from axis_members into this widget or a new model field.)
        self.hoverDataSelector = MultiFieldSelector(self)
        # Optionally, you could initialize its available options/selected fields from your model:
        # e.g., self.hoverDataSelector.set_selected_fields(self.model.hover_data) if you add a hover_data field.
        hover_layout.addRow("Select Hover Data:", self.hoverDataSelector)
        self.content_layout.addWidget(self.hoverDataGroupBox)
        
        # --- Advanced Settings GroupBox ---
        self.advancedSettingsGroupBox = QGroupBox("Advanced Settings", self)
        advanced_layout = QVBoxLayout(self.advancedSettingsGroupBox)
        # Build the advanced settings form section using your existing helper.
        self.advancedSettingsWidget = self.build_form_section(self.model.advanced_settings, "advanced_settings")
        advanced_layout.addWidget(self.advancedSettingsWidget)
        self.content_layout.addWidget(self.advancedSettingsGroupBox)
        ...
        # Initially, both group boxes are hidden since the checkboxes are unchecked.
        self.hoverDataGroupBox.setVisible(False)
        self.advancedSettingsGroupBox.setVisible(False)
        
        # Connect toggle signals.
        self.hoverDataToggle.stateChanged.connect(self.toggleHoverDataGroup)
        self.advancedSettingsToggle.stateChanged.connect(self.toggleAdvancedSettingsGroup)
        
        # Call set_plot_type to initialize visibility for fields that depend on plot type.
        self.set_plot_type(self.current_plot_type)
    
    def toggleHoverDataGroup(self, state):
        """Toggle visibility of the hover data group box based on checkbox state."""
        visible = bool(state)
        self.hoverDataGroupBox.setVisible(visible)
        # Optionally, update the model if you want to record this preference.
    
    def toggleAdvancedSettingsGroup(self, state):
        """Toggle visibility of the advanced settings group box based on checkbox state."""
        visible = bool(state)
        self.advancedSettingsGroupBox.setVisible(visible)
        # Optionally, update the model if you want to record this preference.
    
    def build_form_section(self, section_model, model_attr_name):
        """
        Build a form section for a given sub‑model.
        This method creates a widget with a QFormLayout and stores the field widgets in a dictionary.
        """
        widget = QWidget(self)
        ...
        for f in fields(section_model):
            ...
            # Set initial values and connect signals based on widget type.
            if isinstance(field_widget, QLineEdit):
                ...
                # Populate combobox if needed; here we assume items will be added elsewhere.
    ...
    #             field_widget.setChecked(bool(value))
    #             field_widget.stateChanged.connect(lambda state, fname=f.name, m=section_model: setattr(m, fname, bool(state)))
    #             field_widget.stateChanged.connect(lambda state, curr_plot_type=self.current_plot_type: self.set_plot_type(curr_plot_type))
    #         elif isinstance(field_widget, QComboBox):
    ...
    def set_plot_type(self, plot_type: str):
        """
        Update the visibility of each field in all sections (except for our top‑level toggles and group boxes
        that are controlled independently) based on the current plot type and any dependency rules specified
        in the field's metadata.
        """
        self.current_plot_type = plot_type.lower()
        
        # Iterate over each section that was built by build_form_section.
        # self.section_widgets is a dict keyed by the section name (e.g. "plot_labels", "axis_members", "advanced_settings")
        for section, widgets_dict in self.section_widgets.items():
            # Retrieve the corresponding model for this section.
            section_model = getattr(self.model, section, None)
            # Get the form layout for this section (if available)
            form_layout = self.section_form_layouts.get(section) if hasattr(self, "section_form_layouts") else None
            if section_model is None:
                continue
            
            # Loop over each field widget in the section.
            for fname, widget in widgets_dict.items():
                # Look up the field's metadata from the model.
                field_metadata = None
                for f in fields(section_model):
                    if f.name == fname:
                ...
                # Start with visibility based solely on plot_types.
                visible = True
                if "plot_types" in field_metadata:
                    visible = visible and (self.current_plot_type in field_metadata["plot_types"])
                
                # Next, handle any dependency rules in "depends_on".
                if "depends_on" in field_metadata:
                    dep = field_metadata["depends_on"]
                    if isinstance(dep, list):
                        # Process a list of dependencies.
                        for d in dep:
                            if isinstance(d, str):
                                visible = visible and bool(getattr(section_model, d, False))
                            elif isinstance(d, tuple) and len(d) == 2:
                                # A tuple: (dependent_field, required_value)
                                visible = visible and (getattr(section_model, d[0], None) == d[1])
                    else:
                        visible = visible and bool(getattr(section_model, dep, False))
                
                # Finally, show or hide the widget (and its associated label, if available)
                if visible:
                    widget.show()
                    if form_layout:
