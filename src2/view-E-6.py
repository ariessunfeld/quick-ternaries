...
from cartesian_trace_maker import CartesianContourTraceMaker
...
class TabPanel(QWidget):
    ...
    def add_tab(self, title: str, model) -> str:
        ...
        print(f'[inside TabPanel->add_tab()] getting unique_id took {t1-t0:.4f}')
        print(f'[inside TabPanel->add_tab()] storing model in self.id_to_widget took {t2-t1:.4f}')
...
class TraceEditorView(QWidget):
    ...
    def _build_ui(self):
        ...
        print(f'[inside TraceEditorView->_build_ui] first loop took {t_1-t_0:.4f}')
        for k,v in fields_to_times.items():
            print(f'{k}: {v}s')
        ...
        if getattr(self.model, "is_contour", False):
            ...
            t1 = time()
            print(f'[inside TraceEditorView->_build_ui] self.add_source_point_info() took {t1-t0:.4f}')
            # Add error entry widget
            self.add_error_entry_widget()
            t2 = time()
            print(f'[inside TraceEditorView->_build_ui] self.add_error_entry_widget() took {t2-t1:.4f}')
    ...
    def set_model(self, new_model: TraceEditorModel):
        ...
        print(f'[inside TraceEditorView->set_model()] storing the model took {t1-t0:.4f}')
        ...
        print(f'[inside TraceEditorView->set_model()] removing widgets took {t2-t1:.4f}')
        ...
        print(f'[inside TraceEditorView->set_model()] self._build_ui() took {t3-t2:.4f}')
        ...
        print(f'[inside TraceEditorView->set_model()] self.update_from_model() took {t4-t3:.4f}')
        ...
        print(f'[inside TraceEditorView->set_model()] self.set_plot_type(...) took {t5-t4:.4f}')
        ...
        print(f'[inside TraceEditorView->set_model()] self._build_filters_ui() took {t6-t5:.4f}')
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.cartesian_contour_maker = CartesianContourTraceMaker()
    ...
    #     # Create figure using appropriate plot maker based on plot type
    #     if current_plot_type == 'ternary':
    #         fig = self.ternary_plot_maker.make_plot(self.setupMenuModel, traces_data)
    #     elif current_plot_type == 'cartesian':
    #         fig = self.cartesian_plot_maker.make_plot(self.setupMenuModel, traces_data)
    #     else:
    #         QMessageBox.warning(
    #             self, 
    ...
    #     if filename:
    #         # Use the appropriate plot maker's save_plot method
    #         if current_plot_type == 'ternary':
    #             self.ternary_plot_maker.save_plot(fig, filename, dpi=600)
    #         else:
    #             self.cartesian_plot_maker.save_plot(fig, filename, dpi=600)
    ...
    def on_export_clicked(self):
        ...
        # Save to file
        filename, _ = QFileDialog.getSaveFileName(
            self, "Export Plot", "", "Image Files (*.png *.jpeg *.svg *.jpg *.html)"
        )
        if filename:
            # Use the appropriate plot maker's save_plot method
            if current_plot_type == 'ternary':
    ...
    def on_preview_clicked(self):
        ...
        # For ternary plots only, validate formulas for molar conversion
        if current_plot_type == 'ternary' and not self.validate_formulas_for_molar_conversion():
            return  # Don't proceed with rendering if validation fails
        ...
        # Create the appropriate figure based on plot type
        if current_plot_type == 'ternary':
            # Create the basic figure with regular traces using ternary plot maker
            fig = self.ternary_plot_maker.make_plot(self.setupMenuModel, regular_traces)
            ...
            for uid, model in contour_traces:
                try:
                    ...
                    print(f"Error generating ternary contour for trace {uid}: {str(e)}")
        
        elif current_plot_type == 'cartesian':
            # Create the figure with regular traces using cartesian plot maker
            fig = self.cartesian_plot_maker.make_plot(self.setupMenuModel, regular_traces)
            
            # Add contour traces if any exist for cartesian plots
            for uid, model in contour_traces:
                # try:
                # Generate the contour trace using our new cartesian contour maker
                contour_trace = self.cartesian_contour_maker.make_trace(
                    model, 
                    self.setupMenuModel, 
                    uid
                )
                ...
                if contour_trace:
                    fig.add_trace(contour_trace)
                # except Exception as e:
                #     print(f"Error generating cartesian contour for trace {uid}: {str(e)}")
    ...
    def create_new_tab(self):
        ...
        print(f'\tFinding next trace number took {t2-t1:.4f}')
        print(f'\tInstantiating model took {t3-t2:.4f}')
        ...
        print(f'\tAdding tab to tabPanel took {t5-t4:.4f}')
        print(f'\tSaving current tab data took {t6-t5:.4f}')
        ...
        print(f'\tSetting the model for the traceEditorView took {t8-t7:.4f}')
        print(f'\tShowing the trace editor took {t9-t8:.4f}')
        ...
        print(f'\tDONE ADDING TAB (total time: {t9-t0})')
