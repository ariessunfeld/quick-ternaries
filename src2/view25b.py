...
class TraceEditorController:
    def __init__(self, model: TraceEditorModel, view: TraceEditorView):
        ...
        # Flag to track if this is a user interaction or programmatic update
        self.is_programmatic_update = False

    def on_datafile_changed(self, new_file: str):
        # Skip processing if no file or if it's the same as current model value
        if not new_file or new_file == self.model.datafile:
            return
            
        # If this isn't a programmatic update, it must be user-initiated
        if not self.is_programmatic_update:
            # Check if heatmap is on
            heatmap_on = self.model.heatmap_on
            current_column = self.model.heatmap_column
            
            if heatmap_on and current_column:
                # Get numeric columns from the new datafile
                numeric_cols = get_numeric_columns_from_file(new_file)
                
                # Show warning if current column isn't in the new file
                if current_column not in numeric_cols:
                    QMessageBox.warning(
                        self.view, 
                        "Column Missing",
                        f"The currently selected heatmap column '{current_column}' is not available in the new datafile."
                    )
        
        # Always update the model's datafile value
        self.model.datafile = new_file
        
        # Now update the heatmap column options
        self.update_column_options(new_file)

    def update_column_options(self, file_path: str):
        """Update column options without triggering warnings"""
        numeric_cols = get_numeric_columns_from_file(file_path)
        ...
        if heatmap_combo:
            ...
            # Set value based on availability
...
class MainWindow(QMainWindow):
    ...
    def update_trace_datafile_options(self):
        ...
        if combobox:
            # Mark the controller to handle this as a programmatic update
            self.traceEditorController.is_programmatic_update = True
            
            # Save the current model value
            current_file = self.traceEditorView.model.datafile
            ...
            if current_file and current_file in data_files:
                combobox.setCurrentText(current_file)
            elif data_files:
                # Set to first item (this will trigger on_datafile_changed)
                combobox.setCurrentIndex(0)
            
            # Reset the flag after all updates are complete
            self.traceEditorController.is_programmatic_update = False
