...
class TraceEditorView(QWidget):
    ...
    def _build_ui(self):
        ...
        for f in fields(self.model):
            ...
            if group_name:
                group_fields.setdefault(group_name, []).append((f.name, label_text, widget, metadata))
            else:
                self.form_layout.addRow(label_text, widget)
        # Now handle group boxes.
        for group_name, field_tuples in group_fields.items():
            if group_name == "heatmap":
                # Create a special group box for heatmap.
        ...
        for group_name, (group_box, _) in self.group_boxes.items():
            if group_name == "heatmap":
                ...
                if heatmap_on and hasattr(self, "advanced_heatmap_container"):
                    self.advanced_heatmap_container.setVisible(getattr(self.model, "heatmap_use_advanced", False))
            else:
                group_visible = False
                for f in fields(self.model):
                    metadata = f.metadata
...
class TraceEditorController:
    ...
    def on_datafile_changed(self, new_file: str):
        ...
        if heatmap_combo:
            ...
            # Preserve the saved value if it is still available.
...
class MainWindow(QMainWindow):
    ...
    def _show_trace_editor(self):
        ...
        # Block signals during datafile update to prevent unwanted triggers
        ...
        if datafile_combo:
            ...
            # Force an update of column selections based on the current datafile
            # but only if there's a selected datafile
            current_datafile = self.traceEditorView.model.datafile
            if current_datafile:
                self.traceEditorController.on_datafile_changed(current_datafile)
    ...
    def load_workspace(self):
        ...
        if filename:
            ...
            self.setupMenuView.update_from_model()  # repopulates the data library list.
            self.setupMenuView.set_plot_type(self.setupMenuView.current_plot_type)
            # Reinitialize the SetupMenu Controller.
            self.setupController = SetupMenuController(self.setupMenuModel, self.setupMenuView)
            self.setupMenuView.set_controller(self.setupController)
            
            # Temporarily disconnect the datafile combo's signal to prevent unwanted updates.
            datafile_combo = self.traceEditorView.widgets.get("datafile")
            if datafile_combo:
                try:
                    datafile_combo.currentTextChanged.disconnect(self.traceEditorController.on_datafile_changed)
                except Exception:
                    pass
            # Update the datafile options.
            self.update_trace_datafile_options()
            # Reconnect the signal.
            if datafile_combo:
                datafile_combo.currentTextChanged.connect(self.traceEditorController.on_datafile_changed)
