...
def get_numeric_columns(file_path):
    ...
    try:
        if file_path.lower().endswith('.csv'):
            ...
            return []
        # Select columns with numeric dtypes.
...
class TraceEditorView(QWidget):
    ...
    def _build_ui(self):
        ...
        for f in fields(self.model):
            ...
            if isinstance(widget, QComboBox):
                if f.name == "datafile":
                    # Populate initially using the current model value.
                    # (Assume the list of datafiles will be updated externally via a helper.)
                    widget.setCurrentText(str(value))
                    # Connect the signal to our custom handler.
                    widget.currentTextChanged.connect(self.on_datafile_changed)
                    # Store the current selection.
                    self._datafile_previous = widget.currentText() or ""
                else:
                    widget.addItems([])  # For other combo boxes.
                    widget.setCurrentText(str(value))
    ...
    def update_from_model(self):
        ...
        self._datafile_update_in_progress = True
        for f in fields(self.model):
            widget = self.widgets.get(f.name)
            value = getattr(self.model, f.name)
            if isinstance(widget, QComboBox):
                if f.name == "datafile":
                    # Here we assume that an external helper (from MainWindow) will update the list of datafiles.
                    widget.setCurrentText(str(value))
                    self._datafile_previous = widget.currentText() or ""
                else:
                    widget.setCurrentText(str(value))
                ...
                widget.setChecked(bool(value))
        self._datafile_update_in_progress = False
    ...
    def on_datafile_changed(self, new_datafile):
        # Avoid recursion when updating programmatically.
        if hasattr(self, "_datafile_update_in_progress") and self._datafile_update_in_progress:
            return

        # Obtain numeric columns from the newly selected file.
        numeric_cols = get_numeric_columns(new_datafile)
        heatmap_cb = self.widgets.get("heatmap_column")
        prev_heatmap = self.model.heatmap_column  # previously selected heatmap column
        ...
        if heatmap_cb:
            ...
            # Check if the previously configured heatmap column is still valid.
            if prev_heatmap and prev_heatmap not in numeric_cols:
                reply = QMessageBox.question(
                    self,
                    "Reset Heatmap Config",
                    ("The previously selected heatmap column "
                    f"('{prev_heatmap}') is not available or not numeric in the new datafile.\n"
                    "This will reset your heatmap configuration. Do you want to proceed?"),
                    QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
                )
                if reply == QMessageBox.StandardButton.No:
                    # User canceled: revert the datafile selection.
                    datafile_cb = self.widgets.get("datafile")
                    datafile_cb.setCurrentText(self._datafile_previous)
                    self._datafile_update_in_progress = False
                    return
                else:
                    # User accepted: clear/reset the heatmap configuration smartly.
                    self.model.heatmap_column = ""
                    # Optionally, reset other heatmap settings (min, max, etc.) as needed.
            else:
                # If valid, set the combobox to the previous value.
                if prev_heatmap in numeric_cols:
                    heatmap_cb.setCurrentText(prev_heatmap)
        # End blocking updates.
        self._datafile_update_in_progress = False

        # Update the model with the new datafile and store it as the previous selection.
        self.model.datafile = new_datafile
        self._datafile_previous = new_datafile
