...
class TraceEditorView(QWidget):
    ...
    def update_datafile_options(self, file_list: list):
        widget = self.widgets.get("datafile")
        if widget is not None:
            current = widget.currentText()
            widget.clear()
            widget.addItems(file_list)
            if current in file_list:
                widget.setCurrentText(current)
            else:
                if file_list:
                    widget.setCurrentIndex(0)
                    setattr(self.model, "datafile", file_list[0])
                else:
                    widget.setCurrentText("")
                    setattr(self.model, "datafile", "")
...
class DataLibraryController:
    def __init__(self, data_library_model: DataLibraryModel, setup_view: QWidget, tab_panel: TabPanel, trace_editor_view: TraceEditorView):
        self.data_library_model = data_library_model  # Note: pass the DataLibraryModel instance
        self.setup_view = setup_view
        self.tab_panel = tab_panel
        self.trace_editor_view = trace_editor_view

    def remove_file(self, file_path: str) -> bool:
        # Find all traces that depend on this file.
        dependent_traces = []
        for uid, model in self.tab_panel.id_to_widget.items():
            if isinstance(model, TraceEditorModel) and model.datafile == file_path:
                dependent_traces.append(model.trace_name)
        if dependent_traces:
            message = ("Deleting this file will also delete trace(s): " +
                       ", ".join(dependent_traces) +
                       ".\nTo preserve these trace(s), click Cancel, switch the datafile " +
                       "associated with these traces, and then delete the file from the Data Library.")
            reply = QMessageBox.question(self.setup_view, "Confirm Deletion", message,
                                         QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Cancel)
            if reply == QMessageBox.StandardButton.Cancel:
                return False
            else:
                # Remove traces that depend on the file.
                for uid, model in list(self.tab_panel.id_to_widget.items()):
                    if isinstance(model, TraceEditorModel) and model.datafile == file_path:
                        self.tab_panel.remove_tab_by_id(uid)
        # Remove the file from the data library.
        if file_path in self.data_library_model.loaded_files:
            self.data_library_model.loaded_files.remove(file_path)
        return True

    def update_all_datafile_options(self):
        # Update the datafile combobox in the active trace editor.
        self.trace_editor_view.update_datafile_options(self.data_library_model.loaded_files)
...
class SetupMenuView(QWidget):
    ...
    def add_data_file(self):
        ...
        if file_path:
            ...
            if hasattr(self, "dataLibraryController"):
                self.dataLibraryController.update_all_datafile_options()
    ...
    def remove_data_file(self):
        ...
        if current_item:
            ...
            if hasattr(self, "dataLibraryController"):
                if not self.dataLibraryController.remove_file(file_path):
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.dataLibraryController = DataLibraryController(self.setupMenuModel.data_library, self.setupMenuView, self.tabPanel, self.traceEditorView)
        self.setupMenuView.dataLibraryController = self.dataLibraryController
