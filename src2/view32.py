...
def get_all_columns_from_file(file_path):
    try:
        if file_path.lower().endswith('.csv'):
        ...
        for f in fields(self.filter_model):
            ...
            if isinstance(widget, QLineEdit):
                ...
                if f.name == "filter_operation":
                    ...
                    if datafile:
                        # Always get the latest columns from the current datafile
...
class TraceEditorView(QWidget):
    ...
    def _build_ui(self):
        ...
        for f in fields(self.model):
            ...
            elif isinstance(widget, ColorScaleButton):
                # Handle our custom ColorScaleButton
    ...
    def load_workspace(self):
        ...
        if filename:
            try:
                ...
                # Set the tab selection in the UI, but don't rely on it to update content yet
                self.tabPanel.listWidget.setCurrentRow(0)
                ...
                # CRITICAL: After all loading and fixes are done, explicitly trigger tab selection handler
                # This forces the UI to show the right content
                self.current_tab_id = "setup-menu-id"  # Set this first
                self._show_setup_content()  # Force the content to be the setup menu
