...
class SetupMenuView(QWidget):
    def __init__(self, model: SetupMenuModel, parent=None):
        ...
        self.controller = None  # Will be set by the main window.
        self.section_widgets = {}  # Will hold per‑section widget mappings keyed by model attribute name.
        ...
        # Outer layout for the SetupMenuView contains only the scroll area.
    ...
    def build_form_section(self, section_model, model_attr_name):
        ...
        # Store the form layout so we can later access the labels.
    ...
    def set_plot_type(self, plot_type: str):
        self.current_plot_type = plot_type
        # Iterate over each section (keyed by the model attribute name, e.g. "axis_members").
...
class WorkspaceManager:
    ...
    def __init__(self, traces: list, setup_model: SetupMenuModel):
        """
        :param traces: a list of TraceEditorModel instances
        :param setup_model: a SetupMenuModel instance
        """
        self.traces = traces
    ...
    def from_dict(cls, d: dict):
        ...
        return cls(traces, setup)

    @classmethod
    def load_from_file(cls, filename: str):
        with open(filename, "r") as f:
            d = json.load(f)
        # You can add version checks here for backward compatibility.
...
class MainWindow(QMainWindow):
    ...
    def save_workspace(self):
        # Gather all trace models from the TabPanel (skip the setup-menu tab)
        traces = []
        ...
        workspace = WorkspaceManager(traces, self.setupMenuModel)
        filename, _ = QFileDialog.getSaveFileName(self, "Save Workspace", "", "JSON Files (*.json)")
        if filename:
            # Optionally, you can let the user type in a name via an input dialog here.
    ...
    def load_workspace(self):
        ...
        if filename:
            ...
            # Remove all existing trace tabs (except the setup-menu)
            ...
            # Trigger a refresh of the setup menu (this calls set_plot_type to update visibilities)
