...
class ScatterPlotDialog(QDialog):
    def __init__(self, parent, title, html_path):
        ...
        # Load the URL
        print(f"Loading scatter plot from {html_path}")
        url = QUrl.fromLocalFile(html_path)
        self.webView.setUrl(url)
    
    def onLoadFinished(self, success):
        if success:
            self.statusLabel.setText("Plot loaded successfully")
            print("Scatter plot loaded successfully")
        else:
            self.statusLabel.setText("Error loading plot")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Create a temp directory for our plots
        self.temp_dir = tempfile.mkdtemp(prefix="heatmap_plots_")
        print(f"Created temp directory for plots: {self.temp_dir}")
    ...
    def generate_scatter_plot(self, facies, target, other):
        ...
        # Generate a unique filename for this scatter plot
        scatter_filename = f"scatter_{target}_{other}_{facies_converted}_{int(np.random.random()*10000)}.html"
        scatter_path = os.path.join(self.temp_dir, scatter_filename)
        
        # Write the HTML with full Plotly included
        scatter_html = scatter_fig.to_html(include_plotlyjs=True, full_html=True)
        with open(scatter_path, "w") as f:
            f.write(scatter_html)
        
        # Create and show the dialog with scatter plot
        dialog_title = f"Scatter Plot: {target} vs {other} (Facies {facies_converted})"
        scatter_dialog = ScatterPlotDialog(self, dialog_title, os.path.abspath(scatter_path))
        scatter_dialog.exec()
