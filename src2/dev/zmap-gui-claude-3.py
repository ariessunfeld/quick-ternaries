...
class MainWindow(QMainWindow):
    ...
    def on_load_finished(self, success):
        ...
        if not success or not self.current_target:
            print("Page load failed or no target element")
            return
            
        # Inject JavaScript to set up double-click handlers
        js_code = f"""
        var targetElement = '{self.current_target}';
        console.log("Page loaded, target element:", targetElement);
        ...
        function setupDoubleClick() {{
            ...
            if(myPlot && myPlot.on) {{
                ...
                console.error("Plot element not ready yet");
                setTimeout(setupDoubleClick, 100);
            }}
        }}
        
        // Start setup when Plotly is ready
        if (typeof Plotly !== 'undefined') {{
            console.log("Plotly is available");
            setupDoubleClick();
        }} else {{
            console.error("Plotly not available yet, waiting...");
            var interval = setInterval(function() {{
                if (typeof Plotly !== 'undefined') {{
                    clearInterval(interval);
                    console.log("Plotly became available");
                    setupDoubleClick();
                }}
            }}, 100);
        }}
        ...
        self.webView.page().runJavaScript(js_code, QWebEngineScript.ScriptWorldId.ApplicationWorld)
        print(f"Injected double-click handler JavaScript for target: {self.current_target}")
