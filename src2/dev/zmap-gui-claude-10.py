...
# ---------------------------------------------------------
# ScatterPlotDialog for showing scatter plots.
# ---------------------------------------------------------
class ScatterPlotDialog(QDialog):
    def __init__(self, parent, title, html_content):
        ...
        # Info label
        self.infoLabel = QLabel(title)
        layout.addWidget(self.infoLabel)
        
        # Status label
        self.statusLabel = QLabel("Loading plot...")
        layout.addWidget(self.statusLabel)
        
        # Web view for the scatter plot
        ...
        self.debug_timer.timeout.connect(self.check_plotly)
        self.debug_timer.start(500)  # Check every 500ms
    
    def check_plotly(self):
        """Check if Plotly is loaded in the scatter plot"""
        self.webView.page().runJavaScript(
            ...
            self.on_plotly_check
        )
    
    def on_plotly_check(self, result):
        """Handle the Plotly check result"""
        if result.get('hasPlotly', False) and result.get('hasPlot', False):
            self.statusLabel.setText("Plot data loaded")
            self.debug_timer.stop()
    
    def onLoadFinished(self, success):
        if success:
            self.statusLabel.setText("HTML loaded successfully")
            print("Scatter plot HTML loaded successfully")
            
            # Inject debug code
            self.webView.page().runJavaScript(
                """
                if (typeof Plotly !== 'undefined') {
                    console.log("Plotly is available immediately");
                } else {
                    console.log("Waiting for Plotly to load...");
                    // This will show when Plotly becomes available
                    Object.defineProperty(window, 'Plotly', {
                        configurable: true,
                        enumerable: true,
                        get: function() { return this._plotly; },
                        set: function(v) {
            ...
            self.statusLabel.setText("Error loading HTML")
            print("Error loading scatter plot HTML")
...
class MainWindow(QMainWindow):
    ...
    def generate_scatter_plot(self, facies, target, other):
        ...
        # Generate HTML with minimal overhead
        minimal_html = """
        <!DOCTYPE html>
        <html>
        <head>
        ...
        """.format(
            data=json.dumps(scatter_fig.data, cls=plotly.utils.PlotlyJSONEncoder),
            layout=json.dumps(scatter_fig.layout, cls=plotly.utils.PlotlyJSONEncoder)
        )

        # Create dialog title
        dialog_title = f"Scatter Plot: {target} vs {other} (Facies {facies_converted})"

        # Create the dialog with direct HTML
        scatter_dialog = ScatterPlotDialog(self, dialog_title, minimal_html)
        scatter_dialog.exec()
