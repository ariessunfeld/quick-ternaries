...
from PySide6.QtWidgets import (
    ...
    QFileDialog, QHBoxLayout, QDialog, QLabel, QMessageBox
)
...
# ---------------------------------------------------------
# ScatterPlotDialog with its own QWebEngineProfile
# ---------------------------------------------------------
class ScatterPlotDialog(QDialog):
    def __init__(self, parent, title, html_file_path):
        ...
        # Add status label
        self.statusLabel = QLabel("Loading scatter plot...")
        layout.addWidget(self.statusLabel)
        
        # Create a separate web profile for this dialog
        # Use an OffTheRecord profile to avoid any disk cache conflicts
        self.profile = QWebEngineProfile(f"ScatterProfile_{time.time()}", self)
        # self.profile.isOffTheRecord()
        # self.profile.setProperty('isOffTheRecord')
        # self.profile.set
        
        # Create web view with the custom profile
        self.webView = QWebEngineView()
        self.webPage = self.profile.createPage()
        ...
        # Load the URL (do this after everything is set up)
        url = QUrl.fromLocalFile(html_file_path)
        print(f"Loading scatter plot from: {html_file_path}")
        self.webView.setUrl(url)
    
    def on_load_finished(self, success):
        """Called when the web page finishes loading"""
        if success:
            self.statusLabel.setText("Scatter plot loaded successfully")
            print("Scatter plot loaded successfully")
            
            # Inject debug code to verify Plotly is available
            self.webView.page().runJavaScript(
                """
                if (typeof Plotly !== 'undefined') {
                    console.log("Plotly is available in scatter plot");
                    document.title = "Plotly loaded successfully";
                } else {
                    console.log("Plotly is NOT available in scatter plot");
                    document.title = "Error: Plotly not loaded";
                }
                """
            )
        else:
            self.statusLabel.setText("Error loading scatter plot")
            print("Error loading scatter plot")
...
class MainWindow(QMainWindow):
    ...
    def generate_scatter_plot(self, facies, target, other):
        """Generate a scatter plot (with best-fit line) for the given facies and element pair,
           and display it in a dialog with its own QWebEngineProfile."""
        ...
        if df_f.empty:
            ...
            if not df_f.empty:
                ...
                QMessageBox.warning(self, "No Data", f"No data found for Facies {facies_converted}")
                return
        ...
        try:
            ...
            QMessageBox.warning(self, "Missing Columns", f"Columns {target} and/or {other} not found in CSV.")
            return
        ...
        # Create the dialog with the custom WebEngineProfile
        dialog_title = f"Scatter Plot: {target} vs {other} (Facies {facies_converted})"
        scatter_dialog = ScatterPlotDialog(self, dialog_title, os.path.abspath(filepath))
