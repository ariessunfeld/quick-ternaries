...
class MainWindow(QMainWindow):
    ...
    def generate_scatter_plot(self, facies, target, other):
        ...
        # Create a simple dialog to show the plot
        dialog = QDialog(self)
        dialog.setWindowTitle(f"Scatter Plot: {target} vs {other} (Facies {facies_converted})")
        dialog.resize(800, 600)
        
        # Create layout and add a simple status label
        layout = QVBoxLayout(dialog)
        status_label = QLabel(f"Loading scatter plot for {target} vs {other} (Facies {facies_converted})...")
        layout.addWidget(status_label)
        
        # Create web view for plot
        ...
        def on_plot_loaded(success):
            if success:
                status_label.setText(f"Scatter plot: {target} vs {other} (Facies {facies_converted})")
                print("Scatter plot loaded successfully")
            else:
                status_label.setText(f"Error loading scatter plot")
                print("Error loading scatter plot")
        
        web_view.loadFinished.connect(on_plot_loaded)
