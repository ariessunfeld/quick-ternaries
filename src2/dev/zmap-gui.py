...
# ---------------------------------------------------------
# This is the handler object that will be exposed via QWebChannel.
# It receives notifications from the JS (when a cell is double-clicked)
# and calls back into the main window to generate the scatterplot.
# ---------------------------------------------------------
...
class MainWindow(QMainWindow):
    ...
    def generate_plots(self):
        ...
        for target in elements:
            ...
            # Add a hidden element to store the target element info.
            fig.add_annotation(dict(
                x=0, y=1, xref='paper', yref='paper',
                text=f"<div id='targetElementInfo' style='display:none;'>{target}</div>",
                showarrow=False,
                xanchor='left',
                yanchor='top'
            ))
            ...
            # (We use a simple double-click detection via two quick plotly_click events.)
            js_code = """
<script>
var lastClickTime = 0;
document.getElementById('plot').on('plotly_click', function(data) {
    ...
    if(currentTime - lastClickTime < 300) {
         if(data.points.length > 0) {
              ...
              var targetElement = document.getElementById('targetElementInfo').innerText;
...
"""
            # Generate HTML with Plotly and inject the JavaScript.
    ...
    def generate_scatter_plot(self, facies, target, other):
        ...
        scatter_fig.add_trace(go.Scatter(
            x=x_line, y=y_line, mode='lines', name='Best Fit'
        ))
        ...
        scatter_html = scatter_fig.to_html(include_plotlyjs='cdn', full_html=True, div_id="plot")
        ...
        scatter_dialog.setWindowTitle("Scatter Plot")
