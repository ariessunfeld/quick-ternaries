...
#         """When user selects a different datafile, calculate impacts."""
...
#         # Otherwise, prepare to calculate impacts
#         self.selected_datafile = selected
        
#         # This will be filled in by the parent method that calls this dialog
#         # with calculate_impacts() before showing the dialog
...
#             self.impact_list.addItem(impact)
...
#     """Custom widget for displaying and changing datafiles.
    
#     Consists of a read-only QLineEdit to display the current datafile
#     and a button to open a selection dialog.
#     """
    
#     datafileChanged = Signal(object)  # Signal emitted when datafile changes
...
#         # Create read-only line edit for displaying current datafile
#         self.display = QLineEdit()
#         self.display.setReadOnly(True)
#         self.display.setStyleSheet("background-color: #f8f8f8;")
#         self.display.setMinimumWidth(250)
        
#         # Create change button
#         self.change_button = QPushButton("Change Datafile")
        
#         # Add widgets to layout
#         layout.addWidget(self.display, 1)  # Stretch factor 1 to expand with window
#         layout.addWidget(self.change_button)
        
#         # Connect button click
#         self.change_button.clicked.connect(self.open_selection_dialog)
        
#         # Store current datafile
#         self._datafile = None
#         self._all_datafiles = []
...
#         """Set the current datafile and update display."""
#         self._datafile = datafile
#         if datafile:
#             self.display.setText(str(datafile))
#         else:
...
#         """Set reference to main window for accessing controller."""
#         self.main_window = main_window
    
#     def open_selection_dialog(self):
#         """Open dialog for selecting a new datafile with impact validation."""
...
#         # Calculate impacts of changing datafile
#         if hasattr(self.main_window, "traceEditorController"):
#             controller = self.main_window.traceEditorController
#             selected_index = dialog.datafile_combo.currentIndex()
#             if 0 <= selected_index < len(self._all_datafiles):
#                 potential_datafile = self._all_datafiles[selected_index]
#                 impacts, has_critical = controller.calculate_datafile_change_impacts(potential_datafile)
#                 dialog.set_impacts(impacts, has_critical)
        
#         # Show the dialog
#         if dialog.exec_() == QDialog.Accepted:
#             new_datafile = dialog.get_selected_datafile()
#             if new_datafile and new_datafile != self._datafile:
#                 self._datafile = new_datafile
#                 self.display.setText(str(new_datafile))
...
#     Consists of a read-only QLineEdit to display the current datafile
#     and a small button with ellipsis (...) to open a selection dialog.
#     """
...
#         self.display = QLineEdit()
#         self.display.setReadOnly(True)
#         # Make style match other form elements but indicate it's read-only
#         self.display.setStyleSheet("background-color: rgba(240, 240, 240, 0.5);")
        
#         # Create a small ellipsis button
#         self.change_button = QPushButton("...")
#         self.change_button.setMaximumWidth(30)
#         self.change_button.setToolTip("Select Datafile")
        
#         # Add widgets to layout
#         layout.addWidget(self.display, 1)  # Stretch factor 1 to expand with window
#         layout.addWidget(self.change_button, 0)  # No stretch
...
class DatafileSelector(QWidget):
    ...
    def open_selection_dialog(self):
        ...
        # Pre-calculate impacts for all potential datafiles
        if hasattr(self.main_window, "traceEditorController"):
            controller = self.main_window.traceEditorController
            
            # For each potential datafile, calculate impacts
            for i in range(dialog.datafile_combo.count()):
                potential_datafile = self._all_datafiles[i]
                if potential_datafile != self._datafile:  # Only calculate for different datafiles
                    impacts, has_critical = controller.calculate_datafile_change_impacts(potential_datafile)
                    dialog.potential_impacts[i] = (impacts, has_critical)
            
            # Make sure the impacts for the currently selected datafile are displayed
            dialog.on_selection_changed(dialog.datafile_combo.currentIndex())
        
        # Show the dialog (using exec instead of exec_ to avoid deprecation warning)
...
class MainWindow(QMainWindow):
    ...
    #     # datafile_combo = self.traceEditorView.widgets.get("datafile")
    #     # if datafile_combo:
    #     #     # Save the current model value
    #     #     current_datafile = self.traceEditorView.model.datafile
            
    #     #     # Update options
    #     #     self.update_trace_datafile_options()
            
    #     #     # Ensure combobox matches model (important)
    #     #     if current_datafile:
    #     #         current_display_str = str(current_datafile)
    #     #         index = datafile_combo.findText(current_display_str)
    #     #         if index >= 0:
    #     #             datafile_combo.setCurrentIndex(index)
