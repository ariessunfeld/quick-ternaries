...
from ternary_trace_maker_5 import TernaryContourTraceMaker
...
class ContourInfoWidget(QWidget):
    """
    Widget for displaying the source point information for a contour trace.
    Shows the data from the original point in a table format.
    """
    def __init__(self, parent=None):
        ...
        # Add header
        self.header_label = QLabel("<b>Source Point Data:</b>")
        ...
        # Add a "No data available" label (hidden initially)
        self.no_data_label = QLabel("No source point data available")
    ...
    def refresh_from_series(self, series: pd.Series):
        """
        Update the table view to display data from the provided Series.
        
        Args:
            series: pandas Series containing the point data
        """
        ...
        # Create model from series
        model = PandasSeriesModel(series)
...
class TraceEditorView(QWidget):
    ...
    def _build_ui(self):
            ...
            if isinstance(widget, QLineEdit):
                ...
                if f.name == "line_style":
                    ...
                    widget.addItems(["vertical", "horizontal"])
                     ...
                     # Set up the combo box with contour level options
                    if isinstance(widget, QComboBox):
                        widget.clear()
                        widget.addItems([
                            "Contour: 1-sigma", 
                            "Contour: 2-sigma", 
                            "Contour: Custom percentile"
                        ])
                        widget.setCurrentText(str(value))
        ...
        # Add contour info widget if model is a contour
        if getattr(self.model, "is_contour", False):
            self.contour_info_widget = ContourInfoWidget(self)
            self.form_layout.addRow(self.contour_info_widget)
            
            # Try to get source point data from MainWindow
            main_window = self.window()
            if (hasattr(main_window, 'contour_source_points') and 
                hasattr(main_window, 'current_tab_id') and
                main_window.current_tab_id in main_window.contour_source_points):
                
                source_point = main_window.contour_source_points[main_window.current_tab_id]
                if 'series' in source_point:
                    self.contour_info_widget.refresh_from_series(source_point['series'])
        ...
        for group_name, field_tuples in group_fields.items():
            if group_name == "heatmap":
                ...
                for (group, subgroup), sub_field_tuples in subgroup_fields.items():
                    if group == "heatmap":
                        print(
                            f"Building nested groupbox for {subgroup} with {len(sub_field_tuples)} fields"
                        )
    ...
    def set_plot_type(self, plot_type: str):
        """
        Set the plot type and update visibility of fields accordingly.
        Modified to handle contour-specific fields.
        """
        ...
        for f in fields(self.model):
            ...
            # Determine visibility based on plot type
            visible = ("plot_types" not in metadata) or (
                self.current_plot_type in metadata["plot_types"]
            )
            
            # Additional visibility logic for contour-dependent fields
            ...
            # Special handling for contour_percentile field - only show when Custom percentile is selected
            if f.name == "contour_percentile":
                contour_level = getattr(self.model, "contour_level", "")
                visible = visible and contour_level == "Contour: Custom percentile"
        ...
        # Update contour info widget visibility
        if hasattr(self, 'contour_info_widget'):
            self.contour_info_widget.setVisible(getattr(self.model, "is_contour", False))
    ...
    def set_model(self, new_model: TraceEditorModel):
        """
        Set a new model for the trace editor view.
        Modified to handle contour-specific model properties.
        """
        self.model = new_model
        
        # Rebuild UI to account for model changes
        # Remove old widgets
        ...
        # Build UI with new model
        self._build_ui()
        
        # Update widgets from model
        self.update_from_model()
        
        # Update visibility based on plot type
        self.set_plot_type(self.current_plot_type)
    ...
    def on_datafile_changed(self, new_file: Union[str, DataFileMetadata]):
        ...
        if heatmap_combo:
            ...
            if not value_exists_in_new_file:
                if numeric_cols:
                    print(
                        f"Heatmap column '{current_value}' not in new file, switching to '{numeric_cols[0]}'"
                    )
                ...
                print(
                    f"Keeping existing heatmap column '{current_value}' as it exists in new file"
                )
    ...
    def update_axis_options(self):
        ...
        for field_name, widget in axis_widgets.items():
            if isinstance(widget, MultiFieldSelector):
                print(
                    f"Updating MultiFieldSelector for {field_name} with options: {common_list}"
                )
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Add TernaryContourTraceMaker instance
        self.ternary_contour_maker = TernaryContourTraceMaker()

        # Initialize storage for contour source points
        self.contour_source_points = {}
    ...
    def on_preview_clicked(self):
        ...
        for i in range(self.tabPanel.listWidget.count()):
            ...
            if item and item.text() not in (SETUP_MENU_LABEL, ADD_TRACE_LABEL):
                ...
                model = self.tabPanel.id_to_widget.get(uid)
                if isinstance(model, TraceEditorModel):
                    if getattr(model, "hide_on", False):
                        continue
                        
                    if getattr(model, "is_contour", False):
                        # Handle contour traces separately
        ...
        if contour_traces:
            if not hasattr(self, 'ternary_contour_maker'):
                # Initialize contour maker if not exists
                # from src.services.ternary.ternary_contour_trace_maker import TernaryContourTraceMaker
                self.ternary_contour_maker = TernaryContourTraceMaker()
                
            # Add each contour trace to the figure
            for uid, model in contour_traces:
                # Skip if no source point data available
                if not hasattr(self, 'contour_source_points') or uid not in self.contour_source_points:
                    continue
                ...
                try:
                    ...
                    contour_trace = self.ternary_contour_maker.make_trace(
                        ...
                        self.setupMenuModel, 
                        uid, 
                        self.contour_source_points[uid]
                    )
                    ...
                    # QMessageBox.warning(self, "Contour Error", f"Error generating contour: {str(e)}")
    ...
    def on_bootstrap_clicked(self):
        ...
        trace_data = self._get_trace_data_by_curve_number(curve_number)
        
        if not trace_data or 'dataframe' not in trace_data:
            QMessageBox.warning(
                self, 
                "Data Error", 
                "Could not retrieve data for the selected point."
            )
            return
        
        # Extract the specific row from the dataframe
        df = trace_data['dataframe']
        try:
        ...
        new_label = f"Contour from {trace_data.get('name', 'Unknown')}"
        ...
        model = TraceEditorModel(
            ...
            trace_color=trace_data.get('color', "#1f77b4"),  # Use the source trace color or default
            outline_thickness=2  # Slightly thicker line for contours
        )
        
        # Assign the same datafile from the source trace
        if 'datafile' in trace_data and trace_data['datafile']:
            model.datafile = trace_data['datafile']
        elif self.setupMenuModel.data_library.loaded_files:
            # Fallback to the first datafile if source doesn't have one
            model.datafile = self.setupMenuModel.data_library.loaded_files[0]
        
        # Add source point data to the model for later use
        source_point = {
            'series': series,
        ...
        # Store the source point data for later use when rendering
        if not hasattr(self, 'contour_source_points'):
            self.contour_source_points = {}
        self.contour_source_points[uid] = source_point
        
        # Update the preview immediately to show the contour
        QTimer.singleShot(20, lambda: self._update_contour_preview(uid))

    def _update_contour_preview(self, contour_uid):
        """Updates the preview with the contour trace."""
        if hasattr(self, 'contour_source_points') and contour_uid in self.contour_source_points:
            self.on_preview_clicked()

    def _get_trace_data_by_curve_number(self, curve_number):
        """
        Retrieves the trace data for a given curve number.
        
        Args:
            curve_number: The index of the curve in the plot
            
        Returns:
            Dictionary with trace data including dataframe, name, etc.
        """
        traces_data = []
        
        # Get all trace models
        for i in range(self.tabPanel.listWidget.count()):
            item = self.tabPanel.listWidget.item(i)
            if item and item.text() not in (SETUP_MENU_LABEL, ADD_TRACE_LABEL):
                uid = item.data(Qt.ItemDataRole.UserRole)
                ...
                if isinstance(model, TraceEditorModel) and not getattr(model, "hide_on", False):
                    traces_data.append({
                        ...
                        'datafile': model.datafile,
                        'color': model.trace_color,
                    })
        
        # Match curve_number to the trace data
        # Note: This assumes the ordering in the plot matches the ordering of non-hidden traces
        if 0 <= curve_number < len(traces_data):
            trace_data = traces_data[curve_number]
            
            # Add the dataframe from the datafile
            if trace_data['datafile']:
                df = self.setupMenuModel.data_library.dataframe_manager.get_dataframe_by_metadata(
                    trace_data['datafile']
                )
                if df is not None:
                    trace_data['dataframe'] = df
