...
class TraceEditorView(QWidget):
    ...
    def _build_ui(self):
        ...
        for f in fields(self.model):
            ...
            elif isinstance(widget, QCheckBox) and f.name == "heatmap_on":
                widget.setChecked(bool(value))
                widget.toggled.connect(self.on_heatmap_toggled)
            elif isinstance(widget, QComboBox) and f.name == "datafile":
                # Items for the combobox are updated externally.
                widget.activated.connect(lambda idx, cb=widget: self.on_datafile_changed(cb.currentText()))
            elif isinstance(widget, QComboBox):
    ...
    def on_heatmap_toggled(self, checked: bool):
        # When turning heatmap on, verify that the selected datafile has numeric columns.
        if checked:
            numeric_cols = get_numeric_columns_from_file(self.model.datafile)
            if not numeric_cols:
                QMessageBox.warning(
                    self,
                    "Incompatible Datafile",
                    "The selected datafile does not contain any numeric columns required for the heatmap option."
                )
                ...
                return
        # Save the state and update heatmap options immediately.
        self.model.heatmap_on = checked
    ...
    def on_datafile_changed(self, new_datafile):
        datafile_cb = self.widgets.get("datafile")
        numeric_cols = get_numeric_columns_from_file(new_datafile)

        # Only check for heatmap configuration if heatmap is enabled.
        heatmap_checkbox = self.widgets.get("heatmap_on")
        if heatmap_checkbox and heatmap_checkbox.isChecked():
            ...
            if prev_heatmap_col and prev_heatmap_col not in numeric_cols:
                reply = QMessageBox.question(
                    ...
                    (f"The previously selected heatmap column ('{prev_heatmap_col}') is not available or "
                    "is not numeric in the new datafile. This will reset your heatmap configuration for this trace. Continue?"),
                    QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
                )
                if reply == QMessageBox.StandardButton.No:
                    # Revert selection back to the previous datafile.
                    if datafile_cb:
                        datafile_cb.blockSignals(True)
                        datafile_cb.setCurrentText(self._prev_datafile)
                        datafile_cb.blockSignals(False)
                    return
                else:
                    # Reset all heatmap configuration fields to defaults.
                    self.model.heatmap_column = ""
                    self.model.heatmap_sort_mode = "no change"
                    self.model.heatmap_min = 0.0
                    self.model.heatmap_max = 1.0
                    self.model.heatmap_colorscale = "Viridis"

        # Update the model and heatmap options.
        self.model.datafile = new_datafile
        self.update_heatmap_options()
        self._prev_datafile = new_datafile


    def update_heatmap_options(self):
        # Update the heatmap combobox to show only numeric columns from the current datafile.
        heatmap_cb = self.widgets.get("heatmap_column")
        if heatmap_cb:
            numeric_cols = get_numeric_columns_from_file(self.model.datafile)
            heatmap_cb.clear()
            heatmap_cb.addItems(numeric_cols)
            if self.model.heatmap_column in numeric_cols:
                heatmap_cb.setCurrentText(self.model.heatmap_column)
            elif numeric_cols:
                heatmap_cb.setCurrentIndex(0)
    ...
    def set_model(self, new_model: TraceEditorModel):
        ...
        self._prev_datafile = self.model.datafile  # Store the previous selection.
        self.update_from_model()
        self.set_plot_type(self.current_plot_type)
        self._build_filters_ui()
        # Immediately update heatmap options in case the datafile has a different numeric column set.
        self.update_heatmap_options()
...
class MainWindow(QMainWindow):
    ...
    def create_new_tab(self):
        ...
        # Default to the first datafile from the loaded data.
        default_file = self.setupMenuModel.data_library.loaded_files[0]
        model = TraceEditorModel(trace_name=new_label, datafile=default_file)
        uid = self.tabPanel.add_tab(new_label, model)
        self._save_current_tab_data()
        self.current_tab_id = uid
        self.traceEditorView.set_model(model)
        # Immediately update the heatmap options now that the default datafile is set.
        self.traceEditorView.update_heatmap_options()
        self._show_trace_editor()
