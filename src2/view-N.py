...
from src2.ternary_plot_maker_2 import TernaryPlotMaker
from src2.ternary_trace_maker_4 import TernaryContourTraceMaker, DensityContourMaker
from src2.cartesian_plot_maker import CartesianPlotMaker
from src2.error_entry_model import ErrorEntryModel
from src2.pandas_series_model import PandasSeriesModel
from src2.zmap_plot_maker_2 import ZmapPlotMaker  # used to be zmap_plot_maker.py not zmap_plot_maker_2.py
...
class TraceEditorView(QWidget):
    ...
    def _build_ui(self):
        ...
        # # Handle multiple contours checkbox
        # multiple_contours_checkbox = self.widgets.get("density_contour_multiple")
        # if multiple_contours_checkbox and isinstance(multiple_contours_checkbox, QCheckBox):
        #     # Disconnect any existing connections
        #     try:
        #         multiple_contours_checkbox.stateChanged.disconnect()
        #     except TypeError:
        #         pass
            
        #     # Connect to update visibility
        #     multiple_contours_checkbox.stateChanged.connect(
        #         lambda state: self._update_multiple_contours_visibility(bool(state))
        #     )
        ...
        if density_multiple_checkbox and isinstance(density_multiple_checkbox, QCheckBox):
            ...
            print(f"Connected density_contour_multiple checkbox: {self.model.density_contour_multiple}")

        self._build_filters_ui()

    # # Add this as a completely new method to TraceEditorView
    ...
    #     names_edit = self.widgets.get("density_contour_names")
    ...
    #         # Toggle visibility of multiple contours fields
    #         if percentiles_edit:
    ...
    #         if names_edit:
    #             names_edit.setVisible(enable_multiple)
    #             if form_layout:
    #                 label = form_layout.labelForField(names_edit)
    #                 if label:
    #                     label.setVisible(enable_multiple)
    ...
    # 2. Update the visibility handler in TraceEditorView
    def _update_multiple_contours_visibility(self, enable_multiple):
        """Update visibility of fields based on multiple contours checkbox."""
        # Ensure the model attribute is updated
        if hasattr(self.model, "density_contour_multiple"):
...
class MainWindow(QMainWindow):
    ...
    # Add methods for Zmap navigation
    # def on_prev_zmap_clicked(self):
    #     """Handle click on Previous ZMap button."""
    #     self.zmap_plot_maker.prev_plot()
    #     self.on_preview_clicked()  # Re-render with new target

    # def on_next_zmap_clicked(self):
    #     """Handle click on Next ZMap button."""
    #     self.zmap_plot_maker.next_plot()
    #     self.on_preview_clicked()  # Re-render with new target
    ...
    def _export_as_html_or_image(self, filepath: Path):
        ...
        try:
            ...
            if current_plot_type == 'ternary':
                ...
                #         print(f'\n\n\nFOUND A DENSITY CONTOUR TRACE')
                #         contour_trace = self.density_contour_maker.make_trace(
    ...
    def on_preview_clicked(self):
        ...
        if current_plot_type == 'ternary':
            ...
            # #         )
            # #         if contour_trace:
            # #             print("Successfully created density contour trace")
            # #             fig.add_trace(contour_trace)
            # #         else:
            # #             print("Failed to create density contour trace")
            ...
            #     if getattr(model, "density_contour_on", False):
            #         print(f"Generating density contour for trace: {model.trace_name}")
            #         print(f"Multiple contours enabled: {getattr(model, 'density_contour_multiple', False)}")
            ...
            #         # Get contour traces
            #         result = self.density_contour_maker.make_trace(
            #             self.setupMenuModel, model, f"density-{uid}"
            #         )
            ...
            #                 # Add each contour trace to the figure
            #                 print(f"Adding {len(result)} contour traces")
            #                 for trace in result:
            #                     fig.add_trace(trace)
            #             else:
            #                 # Handle case where a single trace is returned
    ...
    #     print(f"Selected point: curve={curve_number}, index={point_index}")
        
    #     # Use the trace mapping to get the correct UI trace index
    #     if not hasattr(self, 'plotly_trace_mapping') or curve_number not in self.plotly_trace_mapping:
    #         QMessageBox.warning(
    #             self, 
    #             "Selection Error",
    #             f"Could not determine which trace this point belongs to. Please try again."
    #         )
    #         return
        
    #     ui_trace_index = self.plotly_trace_mapping.get(curve_number)
    #     if ui_trace_index is None:
    #         QMessageBox.warning(
    #             self, 
    #             "Selection Error",
    #             "Cannot bootstrap from a contour trace. Please select a point from a data trace."
    #         )
    #         return
            
    #     print(f"Mapped to UI trace index: {ui_trace_index}")
        
    #     # Get visible traces
    #     visible_traces = self._get_visible_traces()
        
    #     if ui_trace_index < 0 or ui_trace_index >= len(visible_traces):
    #         QMessageBox.warning(
    #             self, 
    #             "Selection Error",
    #             f"Invalid trace index: {ui_trace_index}. Please try again."
    #         )
    #         return
        
    #     # Get the source trace data
    #     trace_uid, trace_model = visible_traces[ui_trace_index]
    ...
    #     # Only get data traces (not contour traces) for bootstrapping
    #     data_traces = []
    #     for i in range(self.tabPanel.listWidget.count()):
    #         item = self.tabPanel.listWidget.item(i)
    #         if item and item.text() not in (SETUP_MENU_LABEL, ADD_TRACE_LABEL):
    ...
    #     # Get visible traces to calculate true curve number excluding density contours
    #     visible_trace_count = 0
    #     target_trace_index = -1
        
    #     for i, (trace_uid, trace_model) in enumerate(data_traces):
    #         # Count how many plotly traces this UI trace generates
    #         trace_count = 1  # The main trace
            
    #         # Add counts for density contours if enabled
    #         if getattr(trace_model, "density_contour_on", False):
    #             if getattr(trace_model, "density_contour_multiple", False):
    #                 percentiles_str = getattr(trace_model, "density_contour_percentiles", "60,70,80")
    #                 try:
    #                     percentiles = [float(p.strip()) for p in percentiles_str.split(',') if p.strip()]
    #                     trace_count += len(percentiles) if percentiles else 3
    #                 except ValueError:
    #                     trace_count += 3
    #             else:
    #                 trace_count += 1
            
    #         # Check if the curve_number falls within this trace's range
    #         if visible_trace_count <= curve_number < visible_trace_count + trace_count:
    #             target_trace_index = i
    #             break
                
    #         visible_trace_count += trace_count
        
    #     if target_trace_index < 0 or target_trace_index >= len(data_traces):
    #         QMessageBox.warning(
    #             self, 
    #             "Selection Error",
    #             f"Could not determine which trace contains the selected point. Please try again."
    #         )
    #         return
        
    #     # Get the source trace data using the calculated index
    #     trace_uid, trace_model = data_traces[target_trace_index]
    #     print(f"Selected trace: {trace_model.trace_name} (index {target_trace_index})")
    ...
    def on_bootstrap_clicked(self):
        ...
        # try:
        # customdata = point_info.get('customdata')
        # if customdata and len(customdata) > 0:
        #     source_trace_id = customdata[-1]  # Last element contains source trace ID
        # except Exception as e:
        #     print(f"Error extracting source_trace_id: {e}")
