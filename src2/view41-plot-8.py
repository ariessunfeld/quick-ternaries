...
class TabPanel(QWidget):
    def __init__(self, parent=None):
        ...
        self.tabVisibilityToggleCallback = None  # (unique_id, is_visible) -> ...
        ...
        painter.setPen(Qt.GlobalColor.black)
        # Draw eye
        painter.drawEllipse(3, 6, 10, 5)
        painter.drawEllipse(7, 6, 2, 4)
        painter.end()
        self.visibleIcon.addPixmap(pm)
    ...
    def add_tab(self, title: str, model) -> str:
        ...
        # Set initial visibility icon based on model's visible_on property
        visible_on = getattr(model, 'visible_on', True)
        
        # Set tag to store visibility state
        new_item.setData(Qt.ItemDataRole.UserRole + 1, visible_on)
        
        # Set appropriate icon based on visibility
        if visible_on:
            new_item.setIcon(self.visibleIcon)
        else:
            new_item.setIcon(self.hiddenIcon)
    ...
    def _on_item_clicked(self, item: QListWidgetItem):
        ...
        # Check if click is on eye icon
        if self._clicked_on_eye_icon(item):
            uid = item.data(Qt.ItemDataRole.UserRole)
            current_visible = item.data(Qt.ItemDataRole.UserRole + 1)
            
            # Toggle visibility
            new_visible = not current_visible
            item.setData(Qt.ItemDataRole.UserRole + 1, new_visible)
            
            # Update icon
            if new_visible:
                item.setIcon(self.visibleIcon)
            else:
                item.setIcon(self.hiddenIcon)
            
            # Call the visibility callback if defined
            if self.tabVisibilityToggleCallback:
                self.tabVisibilityToggleCallback(uid, new_visible)
                
            return
        
        # Continue with existing click handling
    ...
    def _clicked_on_eye_icon(self, item: QListWidgetItem) -> bool:
    ...
    def _clicked_on_remove_icon(self, item: QListWidgetItem) -> bool:
        ...
        item_rect = self.listWidget.visualItemRect(item)
        icon_size = 16
        # Adjust margin to account for the eye icon as well (moved to the right)
        margin_left = 40
        icon_left = item_rect.left() + margin_left
        icon_top = item_rect.top() + (item_rect.height() - icon_size) // 2
        icon_rect = QRect(icon_left, icon_top, icon_size, icon_size)
    ...
    # Method to update tab visibility from outside
    def set_tab_visibility(self, unique_id: str, is_visible: bool):
        for i in range(self.listWidget.count()):
            it = self.listWidget.item(i)
            if it is not None:
                item_id = it.data(Qt.ItemDataRole.UserRole)
                if item_id == unique_id:
                    it.setData(Qt.ItemDataRole.UserRole + 1, is_visible)
                    if is_visible:
                        it.setIcon(self.visibleIcon)
                    else:
                        it.setIcon(self.hiddenIcon)
                    break
...
class FilterModel:
    ...
    ALLOWED_NUMERIC_OPERATIONS = ["<", ">", "<=", ">=" "==", "a < x < b", "a <= x < b", "a < x <= b", "a <= x <= b"]
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.tabPanel.tabVisibilityToggleCallback = self.on_tab_visibility_toggled
    ...
    def on_preview_clicked(self):
        ...
        self.plotView.setUrl(QUrl.fromLocalFile(Path(__file__).parent / 'test.html'))
    ...
    def load_workspace(self):
        ...
        if filename:
            try:
                ...
                # Instead of replacing the model, update its values
                # (1) Update data_library
                self.setupMenuModel.data_library.loaded_files = []  # Clear existing files
                ...
                for section_name in ['plot_labels', 'axis_members', 'column_scaling', 'advanced_settings']:
                    ...
                    for f in fields(loaded_section):
                        setattr(current_section, f.name, getattr(loaded_section, f.name))
