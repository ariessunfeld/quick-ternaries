...
class TraceEditorView(QWidget):
    def __init__(self, model: TraceEditorModel, parent=None):
        ...
        # Internal flag to suppress handling datafile changes during programmatic updates.
        self._suppress_datafile_change = False
    ...
    def on_heatmap_toggled(self, checked: bool):
        setattr(self.model, "heatmap_on", checked)
        if checked:
            current_file = self.model.datafile
            numeric_cols = get_numeric_columns_from_file(current_file)
            if not numeric_cols:
                QMessageBox.warning(
                    self,
                    "Heatmap Warning",
                ...
                checkbox = self.widgets.get("heatmap_on")
                if checkbox:
                    checkbox.blockSignals(True)
                    checkbox.setChecked(False)
                    checkbox.blockSignals(False)
                setattr(self.model, "heatmap_on", False)
                return
            # Populate the heatmap column combobox with the numeric columns.
            heatmap_col_combobox = self.widgets.get("heatmap_column")
            if heatmap_col_combobox:
                heatmap_col_combobox.blockSignals(True)
                heatmap_col_combobox.clear()
                heatmap_col_combobox.addItems(sorted(numeric_cols))
                heatmap_col_combobox.blockSignals(False)
        else:
            # Optionally clear heatmap configuration when toggled off.
        ...
        for f in fields(self.model):
            ...
            if isinstance(widget, QLineEdit):
                ...
                if f.name == "line_style":
                    ...
                    # For datafile, only connect our custom handler.
                    widget.currentTextChanged.connect(self.on_datafile_changed)
                elif f.name == "heatmap_column":
                    # This combobox is managed programmatically.
                    widget.addItems([])
                    widget.setCurrentText(str(value))
                else:
                    widget.addItems([])
    ...
    def on_datafile_changed(self, new_file: str):
        # If we're suppressing changes (i.e. during a programmatic update), do nothing.
        if self._suppress_datafile_change:
            return
        ...
        # Get numeric columns from the new file.
        numeric_cols = get_numeric_columns_from_file(new_file)

        # Edge Case 1: Heatmap is on but new file has no numeric columns.
        if self.model.heatmap_on and not numeric_cols:
            ...
            datafile_combobox = self.widgets.get("datafile")
            if datafile_combobox:
                datafile_combobox.blockSignals(True)
                datafile_combobox.setCurrentText(old_file)
                datafile_combobox.blockSignals(False)
            return

        # Edge Case 2: Heatmap is on and a heatmap column was selected, but it is invalid in the new file.
        prev_heatmap_col = self.model.heatmap_column
        if self.model.heatmap_on and prev_heatmap_col and prev_heatmap_col not in numeric_cols:
            ret = QMessageBox.question(
                self,
                "Reset Heatmap Config",
                "Changing the datafile will reset your heatmap configuration because the previously selected column is "
                "either missing or not numeric in the new datafile.\nDo you want to proceed?",
            ...
            if ret == QMessageBox.StandardButton.No:
                ...
                # Clear the heatmap configuration.
                self.model.heatmap_column = ""
                heatmap_col_combobox = self.widgets.get("heatmap_column")
                if heatmap_col_combobox:
                    heatmap_col_combobox.blockSignals(True)
        ...
        # Now, update the model with the new file.
        self.model.datafile = new_file

        # Finally, update the heatmap column combobox with the new numeric columns.
        heatmap_col_combobox = self.widgets.get("heatmap_column")
        if heatmap_col_combobox:
            heatmap_col_combobox.blockSignals(True)
            heatmap_col_combobox.clear()
            ...
            if self.model.heatmap_column in numeric_cols:
                heatmap_col_combobox.setCurrentText(self.model.heatmap_column)
            heatmap_col_combobox.blockSignals(False)
    ...
    def update_from_model(self):
        # Suppress datafile change events during update.
        self._suppress_datafile_change = True
        for f in fields(self.model):
            metadata = f.metadata
            if "widget" not in metadata or metadata["widget"] is None:
                continue
...
class MainWindow(QMainWindow):
    ...
    def update_trace_datafile_options(self):
        ...
        if combobox:
            current = self.traceEditorView.model.datafile
            combobox.clear()
            combobox.addItems(data_files)
            # If the current value isn’t valid, default to the first file.
            if current not in data_files and data_files:
                combobox.setCurrentText(data_files[0])
                self.traceEditorView.model.datafile = data_files[0]
            elif current in data_files:
