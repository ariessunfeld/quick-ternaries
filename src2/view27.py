...
class TraceEditorModel:
    ...
    # New toggle for advanced heatmap settings.
    heatmap_use_advanced: bool = field(
        default=False,
        metadata={"label": "Use advanced settings", "widget": QCheckBox, "plot_types": ["ternary", "cartesian"], "group": "heatmap"}
    )
    ...
    # Advanced fields now require both heatmap_on and heatmap_use_advanced to be true.
...
class TraceEditorView(QWidget):
    ...
    def _evaluate_dependencies(self, metadata):
        visible = True
    ...
    def set_plot_type(self, plot_type: str):
        ...
        for f in fields(self.model):
            ...
            visible = visible and self._evaluate_dependencies(metadata)
            if visible:
                widget.show()
                if label:
                    label.show()
        ...
        for group_name, (group_box, group_layout) in self.group_boxes.items():
            ...
            for f in fields(self.model):
                ...
                field_visible = field_visible and self._evaluate_dependencies(metadata)
                if field_visible:
                    group_visible = True
                    break
            if group_visible:
