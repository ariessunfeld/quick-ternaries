...
class TernaryTraceMaker:
    ...
    def _apply_scale_factors(self, df: pd.DataFrame, scale_map: Dict[str, float]) -> pd.DataFrame:
        """
    ...
    def _get_hover_data_and_template(self, setup_model, trace_model, data_df: pd.DataFrame, 
                                    top_columns: List[str], left_columns: List[str], 
                                    right_columns: List[str], scaling_map: Dict[str, float]) -> Tuple[np.ndarray, str]:
        """
        ...
        hovertemplate = "".join(
            f"<br><b>{f'{scaling_map[header]}×' if header in scaling_map and scaling_map[header] != 1 else ''}{header}:</b> %{{customdata[{i}]}}"
    ...
    def molar_conversion(self, formula_map=None) -> pd.DataFrame:
        ...
        for col in apex_columns:
            if col in molar_mapping:
                ...
                self.trace_data_df[molar_col_name] = self.trace_data_df[col] / molar_mass
        ...
        for apex_name, apex_cols_list in zip(
            ...
            self.trace_data_df[self.apex_pattern.format(apex=apex_name, us=self.unique_str)] = \
                self.trace_data_df[[self.MOLAR_PATTERN.format(col=c, us=self.unique_str) 
                                  for c in apex_cols_list if c in molar_mapping]].sum(axis=1)
