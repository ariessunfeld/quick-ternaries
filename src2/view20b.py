...
# -----------------------------
# New Data File and Library Models
# -----------------------------
@dataclass
class DataFileModel:
    filepath: str
    ...
    def load_data(self):
        ...
        if self.filepath.lower().endswith('.csv'):
            df = pd.read_csv(self.filepath, header=self.header)
        elif self.filepath.lower().endswith(('.xls', '.xlsx')):
            df = pd.read_excel(self.filepath, sheet_name=self.sheet, header=self.header)
        else:
...
class DataLibraryModel:
    files: List[DataFileModel] = field(default_factory=list)

    def add_file(self, filepath: str, sheet: str = None, header: int = 0, parse: bool = True):
        df_model = DataFileModel(filepath, sheet, header)
        if parse:
            df_model.load_data()
        self.files.append(df_model)

    def remove_file(self, filepath: str, sheet: str = None):
        # Remove file with matching filepath and sheet
        self.files = [df for df in self.files if not (df.filepath == filepath and df.sheet == sheet)]

    def get_file_options(self) -> List[str]:
        # Return a list of display names (or simply filepaths)
        return [df.filepath for df in self.files]

    def get_file_by_path(self, filepath: str) -> Union[DataFileModel, None]:
...
class TraceEditorModel:
    ...
    sizemap_column: str = field(
        default="",
        metadata={"label": "Sizemap Column:", "widget": QComboBox, "plot_types": ["ternary", "cartesian"], "depends_on": "sizemap_on", "group": "sizemap"}
    )
    sizemap_scale: str = field(
        default="linear",
        metadata={"label": "Sizemap Scale:", "widget": QComboBox, "plot_types": ["ternary", "cartesian"], "depends_on": "sizemap_on", "group": "sizemap"}
    )
...
# -----------------------------
# Update TraceEditorView to sync datafile combobox with DataLibraryModel
# -----------------------------
class TraceEditorView(QWidget):
    def __init__(self, model: TraceEditorModel, data_library: DataLibraryModel, parent=None):
        super().__init__(parent)
        self.model = model
        self.data_library = data_library  # Dependency injected reference to the data library
    ...
    def _build_ui(self):
        ...
        for f in fields(self.model):
            ...
            if f.name == "datafile":
                # For datafile, use a combobox populated from self.data_library.
                widget.clear()
                options = self.data_library.get_file_options()
                widget.addItems(options)
                widget.setCurrentText(value)
                widget.currentTextChanged.connect(lambda text, fname=f.name: self._on_datafile_changed(text))
                ...
                if f.name in ("heatmap_on", "sizemap_on", "filters_on"):
                ...
                elif f.name in ("heatmap_sort_mode", "heatmap_colorscale"):
                    if f.name == "heatmap_sort_mode":
        ...
        # (Filters UI is built separately.)
    ...
    def _on_datafile_changed(self, new_file: str):
        old_file = self.model.datafile
        # Retrieve new file's DataFileModel from self.data_library.
        new_df = self.data_library.get_file_by_path(new_file)
        if new_df is not None:
            new_df.load_data()
            # Example: check heatmap_column validity.
            current_hm = self.model.heatmap_column
            if current_hm and current_hm not in new_df.numeric_columns:
                ret = QMessageBox.warning(self, "Datafile Change Warning",
                    f"The new file does not contain the previously selected heatmap column '{current_hm}'.\n"
                    "Proceeding will reset the heatmap configuration.",
                    QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Cancel)
                if ret == QMessageBox.StandardButton.Cancel:
                    self.widgets["datafile"].blockSignals(True)
                    self.widgets["datafile"].setCurrentText(old_file)
                    self.widgets["datafile"].blockSignals(False)
                    return
                else:
                    self.model.heatmap_column = ""
                    if "heatmap_column" in self.widgets:
                        self.widgets["heatmap_column"].clear()
        self.model.datafile = new_file
        # Optionally, update other dependent fields here.
    ...
    def update_from_model(self):
        for f in fields(self.model):
            ...
            widget = self.widgets.get(f.name)
            value = getattr(self.model, f.name)
            # Special handling for datafile combobox:
            if f.name == "datafile" and isinstance(widget, QComboBox):
                # Re-populate combobox with available file options from the data library.
                options = self.data_library.get_file_options()
                widget.blockSignals(True)
                widget.clear()
                widget.addItems(options)
                # Set the current selection to the model value.
                widget.setCurrentText(str(value))
    ...
    # --- Filters UI Methods (assumed implemented from previous patches) ---
    def _build_filters_ui(self):
        # For brevity, assume your complete filters UI code (building a Filters group box with a FilterTabWidget and FilterEditorView)
        # is implemented here. Ensure that after building, if no filters exist then currentFilterIndex is set to None.
        # Example:
    ...
    def on_filter_add_requested(self):
        ...
        self.filterTabWidget.setCurrentRow(self.currentFilterIndex)
        self._show_current_filter()
...
class SetupMenuController:
    ...
    def update_axis_options(self):
        common_columns = None
        for file_path in self.model.data_library.get_file_options():
...
class SetupMenuView(QWidget):
    ...
    def add_data_file(self):
        ...
        if file_path:
            self.model.data_library.add_file(file_path)
    ...
    def remove_data_file(self):
        ...
        if current_item:
            file_to_remove = current_item.text()
            # Check if any trace depends on this file.
            main_win = self.parent().parent()  # Assumes SetupMenuView is in centerStack, which is child of MainWindow.
            dependent_traces = []
            for uid, model in main_win.tabPanel.id_to_widget.items():
                if isinstance(model, TraceEditorModel) and model.datafile == file_to_remove:
                    dependent_traces.append(model.trace_name)
            if dependent_traces:
                msg = ("Warning: the file you are about to remove is used by trace(s): " +
                    ", ".join(dependent_traces) +
                    ". To preserve these traces, click Cancel and change their datafile.")
                ret = QMessageBox.warning(self, "Dependent Trace Warning", msg,
                                        QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Cancel)
                if ret == QMessageBox.StandardButton.Cancel:
                    return
            row = self.dataLibraryList.row(current_item)
            self.dataLibraryList.takeItem(row)
            # Remove from the data library.
            self.model.data_library.files = [df for df in self.model.data_library.files if df.filepath != file_to_remove]
            if self.controller:
                self.controller.update_axis_options()
    ...
    def update_from_model(self):
        ...
        for path in self.model.data_library.files:
            self.dataLibraryList.addItem(path['filepath'])
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setupMenuView = SetupMenuView(self.setupMenuModel, self)
        self.setupMenuView.main_window = self
        ...
        self.traceEditorView = TraceEditorView(self.current_trace_model, self.setupMenuModel.data_library)
