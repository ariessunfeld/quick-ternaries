...
class TraceEditorController(QObject):
    def __init__(self, model: TraceEditorModel, view: TraceEditorView):
        super().__init__(view)
        self.model = model
        self.view = view
        # Save the current datafile value.
        self.previous_datafile = self.model.datafile
        # Flag to determine if the change was user initiated.
        self.user_change = False
        # Flag to temporarily block processing.
        self._block_datafile_signal = False
        
        self.datafile_combo = view.widgets.get("datafile")
        if self.datafile_combo:
            # Install an event filter to capture user clicks.
            self.datafile_combo.installEventFilter(self)
            # Connect to currentIndexChanged for when the selection actually changes.
            self.datafile_combo.currentIndexChanged.connect(self.on_datafile_changed)

    def eventFilter(self, obj, event):
        # When the datafile combobox gets a mouse press,
        # mark that the next change is user initiated.
        if obj == self.datafile_combo and event.type() == QEvent.Type.MouseButtonPress:
            self.user_change = True
        return super().eventFilter(obj, event)

    def on_datafile_changed(self, index: int):
        # If we're programmatically updating the combobox, ignore this signal.
        if self._block_datafile_signal:
            return

        # If the change wasn’t initiated by the user, simply update our previous value.
        if not self.user_change:
            self.previous_datafile = self.datafile_combo.currentText()
            return

        new_file = self.datafile_combo.itemText(index)

        # If heatmap is on, check that the currently selected heatmap column exists in the new file.
        if self.model.heatmap_on:
            numeric_cols = get_numeric_columns_from_file(new_file)
            current_heatmap = self.model.heatmap_column
            if current_heatmap not in numeric_cols:
                # Warn the user and revert the change.
                QMessageBox.warning(
                    self.view,
                    "Warning",
                    "The currently selected heatmap column is not present in the new datafile. Datafile change rejected."
                )
                self._block_datafile_signal = True
                # Revert to the previous datafile.
                prev_index = self.datafile_combo.findText(self.previous_datafile)
                if prev_index >= 0:
                    self.datafile_combo.setCurrentIndex(prev_index)
                self._block_datafile_signal = False
                self.user_change = False
                return

        # Accept the change: update the model and previous value.
        self.model.datafile = new_file
        self.previous_datafile = new_file
        self.user_change = False
        self.update_heatmap_options(new_file)

    def update_heatmap_options(self, new_file: str):
        numeric_cols = get_numeric_columns_from_file(new_file)
        ...
        if heatmap_combo:
            ...
            # If the model’s heatmap column is still available, keep it; otherwise, default.
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.traceEditorController = TraceEditorController(self.current_trace_model, self.traceEditorView)
