...
class TraceEditorView(QWidget):
    def __init__(self, model: TraceEditorModel, parent=None):
        ...
        # This property must be set by MainWindow after creation:
        self.data_library = None  # Should point to your DataLibrary instance
    ...
    def _build_ui(self):
        ...
        for f in fields(self.model):
            ...
            # Special handling for the datafile field.
            if f.name == "datafile":
                # The datafile widget is now a combobox.
                # Populate options from the shared data library.
                available = self._get_available_datafiles()
                widget.clear()
                widget.addItems(available)
                widget.setCurrentText(str(value))
                widget.currentTextChanged.connect(self._on_datafile_changed)
                ...
                if f.name == "heatmap_on" or f.name == "sizemap_on":
    ...
    def _get_available_datafiles(self) -> list:
        # Returns a list of display names from the shared data library.
        if self.data_library is not None:
            return [entry[0] for entry in self.data_library.get_all_filenames()]
        return []

    def _on_datafile_changed(self, new_file: str):
        previous = self.model.datafile
        if new_file == previous:
            return
        # Check that the new file exists in the library.
        df_obj = None
        if self.data_library is not None:
            for entry in self.data_library.get_all_filenames():
                if entry[0] == new_file:
                    # entry is (display_name, sheet, path)
                    df_obj = self.data_library.get_data(entry[2], entry[1])
                    break
        if df_obj is None:
            QMessageBox.warning(self, "Invalid Datafile", "The selected datafile is not available. Reverting selection.")
            self._update_datafile_combobox(previous)
            return
        # Check dependent fields (example: heatmap_column).
        if self.model.heatmap_on:
            numeric_cols = [col for col in df_obj.get_columns() if str(df_obj.get_dtype(col)).startswith(("float", "int"))]
            if self.model.heatmap_column not in numeric_cols:
                reply = QMessageBox.warning(
                    self, "Incompatible Datafile",
                    f"Warning: the new datafile does not contain column '{self.model.heatmap_column}' required for the current heatmap configuration.\n"
                    "If you proceed, the heatmap configuration will be reset.",
                ...
                if reply == QMessageBox.StandardButton.Cancel:
                    ...
                    self.set_plot_type(self.current_plot_type)
        # (Similarly, update sizemap or filter-dependent fields if needed.)
        self.model.datafile = new_file

    def _update_datafile_combobox(self, file_value: str):
        widget = self.widgets.get("datafile")
        if widget and isinstance(widget, QComboBox):
            available = self._get_available_datafiles()
            widget.blockSignals(True)
            widget.clear()
            widget.addItems(available)
            if file_value in available:
                widget.setCurrentText(file_value)
            else:
    ...
    def set_plot_type(self, plot_type: str):
        ...
        # Also update the datafile combobox items, in case the library changed.
        self._update_datafile_combobox(self.model.datafile)
    ...
    def _build_filters_ui(self):
        ...
        # Left: Instantiate our FilterTabWidget.
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.traceEditorView.data_library = self.setupMenuModel.data_library  # Special set
