...
def get_numeric_columns_from_file(file_path):
    """Read the file and return a list of numeric column names."""
    try:
...
class TraceEditorView(QWidget):
    def __init__(self, model: TraceEditorModel, parent=None):
        ...
        self._datafile_updating = False  # flag to block unwanted signals
    ...
    def _build_ui(self):
        ...
        for f in fields(self.model):
            ...
            if isinstance(widget, QLineEdit):
                ...
                # Re‑add extra connections so that dependent UI parts update.
                if f.name == "heatmap_on":
                    widget.stateChanged.connect(lambda _: self.set_plot_type(self.current_plot_type))
                if f.name == "sizemap_on":
                    widget.stateChanged.connect(lambda _: self.set_plot_type(self.current_plot_type))
                if f.name == "filters_on":
                    widget.stateChanged.connect(lambda _: self._update_filters_visibility())
            elif isinstance(widget, QComboBox):
                # Special handling for datafile field.
                if f.name == "datafile":
                    widget.addItems([])  # will be populated later
                    # Connect our custom slot:
                    widget.currentTextChanged.connect(self.on_datafile_changed)
    ...
    def on_datafile_changed(self, new_datafile):
        """Slot triggered when the datafile combobox changes."""
        if self._datafile_updating:
            return  # Skip processing if we are updating programmatically.
        
        old_datafile = self.model.datafile
        if new_datafile == old_datafile:
            return

        numeric_cols = get_numeric_columns_from_file(new_datafile)

        # Edge Case 1: Heatmap is enabled but no numeric columns found.
        if self.model.heatmap_on and not numeric_cols:
            QMessageBox.warning(self, "Incompatible Datafile",
                                "The selected datafile does not contain any numeric columns,\n"
                                "so it cannot be used with the heatmap option.")
            # Revert the heatmap checkbox.
            heatmap_checkbox = self.widgets.get("heatmap_on")
            if heatmap_checkbox:
                heatmap_checkbox.blockSignals(True)
                heatmap_checkbox.setChecked(False)
                heatmap_checkbox.blockSignals(False)
            self.model.heatmap_on = False
            # Clear heatmap column options.
        ...
        # Edge Case 2: Previously selected heatmap column is invalid in the new file.
        prev_heatmap_col = self.model.heatmap_column
        if prev_heatmap_col and prev_heatmap_col not in numeric_cols:
            reply = QMessageBox.question(
                self,
                "Reset Heatmap Configuration",
                "The previously selected heatmap column is not available or not numeric in the new datafile.\n"
                "Changing the file will reset the heatmap configuration for this trace. Continue?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
            )
            if reply == QMessageBox.StandardButton.No:
                # Revert the datafile combobox to the old selection.
                self._datafile_updating = True
                datafile_combo = self.widgets.get("datafile")
                if datafile_combo:
                    datafile_combo.blockSignals(True)
                    datafile_combo.setCurrentText(old_datafile)
                    datafile_combo.blockSignals(False)
                self._datafile_updating = False
                return
            else:
                # Reset heatmap configuration smartly.
                self.model.heatmap_column = numeric_cols[0] if numeric_cols else ""
                heatmap_combo = self.widgets.get("heatmap_column")
                if heatmap_combo:
                    heatmap_combo.blockSignals(True)
                    heatmap_combo.clear()
                    heatmap_combo.addItems(numeric_cols)
                    heatmap_combo.setCurrentText(self.model.heatmap_column)
                    heatmap_combo.blockSignals(False)

        # Finally, update the heatmap column options regardless.
        heatmap_combo = self.widgets.get("heatmap_column")
        if heatmap_combo:
            heatmap_combo.blockSignals(True)
            heatmap_combo.clear()
            if numeric_cols:
                heatmap_combo.addItems(numeric_cols)
                # Ensure the model value is valid.
                if self.model.heatmap_column not in numeric_cols:
                    self.model.heatmap_column = numeric_cols[0]
                heatmap_combo.setCurrentText(self.model.heatmap_column)
            heatmap_combo.blockSignals(False)

        # Update the model's datafile.
        self.model.datafile = new_datafile

    def update_datafile_options(self, data_files):
        """Update the datafile combobox from a given list, blocking signals during update."""
        datafile_combo = self.widgets.get("datafile")
        if datafile_combo:
            self._datafile_updating = True
            current = datafile_combo.currentText()
            datafile_combo.clear()
            datafile_combo.addItems(data_files)
            # Try to re‑select the model's value.
            model_value = self.model.datafile
            if model_value and model_value in data_files:
                datafile_combo.setCurrentText(model_value)
            elif data_files:
                datafile_combo.setCurrentIndex(0)
                self.model.datafile = data_files[0]
            self._datafile_updating = False
...
class MainWindow(QMainWindow):
    ...
    def update_trace_datafile_options(self):
        # Retrieve the list of loaded data files from the setup menu
        data_files = self.setupMenuModel.data_library.loaded_files
        combobox = self.traceEditorView.widgets.get("datafile")
        if combobox:
            # Get the saved value from the model
            model_value = self.traceEditorView.model.datafile
            combobox.clear()
            combobox.addItems(data_files)
            # If the model's datafile is among the loaded files, select it
            if model_value and model_value in data_files:
                combobox.setCurrentText(model_value)
            # Otherwise, if there are any data files, select the first one
            elif data_files:
    ...
    def _show_trace_editor(self):
        ...
        # Update the datafile combobox before showing the trace editor.
