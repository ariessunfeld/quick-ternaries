...
class TraceEditorController:
    def __init__(self, model: TraceEditorModel, view: TraceEditorView):
        self.model = model
        self.view = view
        self.updating_programmatically = False  # Block updates during reversion
        self.userInitiated = False             # Flag for user-initiated changes
        self.last_datafile = None              # To store the previous datafile
        self.previous_heatmap = None           # To store the previously selected heatmap column

        # Assume that view.widgets["datafile"] is a standard QComboBox
        self.datafile_combo = view.widgets.get("datafile")
        if self.datafile_combo:
            # Store initial values
            self.last_datafile = self.datafile_combo.currentText()
            self.previous_heatmap = self.model.heatmap_column
            # Connect signals
            self.datafile_combo.activated.connect(self.on_datafile_activated)
            self.datafile_combo.currentTextChanged.connect(self.on_datafile_changed)

    def on_datafile_activated(self, index: int):
        # Mark that the user is initiating a change.
        self.userInitiated = True
        # Save the current (old) heatmap column from the model
        self.previous_heatmap = self.model.heatmap_column
    ...
    def on_datafile_changed(self, new_file: str):
        ...
        # If this is a user-initiated change and the heatmap is on,
        # check whether the new file's numeric columns contain the previously selected heatmap column.
        if self.userInitiated and self.model.heatmap_on:
            numeric_cols = get_numeric_columns_from_file(new_file)
            if self.previous_heatmap and self.previous_heatmap not in numeric_cols:
                reply = QMessageBox.warning(
                    self.view,
                    "Warning",
                    f"The new datafile does not include the previously selected heatmap column '{self.previous_heatmap}'.\n"
                ...
                if reply == QMessageBox.StandardButton.No:
                    ...
                    self.datafile_combo.setCurrentText(self.last_datafile)
                    self.model.datafile = self.last_datafile
                    self.updating_programmatically = False
                    self.userInitiated = False
                    return

        # Proceed to update the heatmap options.
        numeric_cols = get_numeric_columns_from_file(new_file)
        ...
        if heatmap_combo:
            ...
            # If the old heatmap column is still present, keep it;
            # otherwise, choose a default if available.
        ...
        # Update the stored datafile and model.
        self.last_datafile = new_file
