...
class TraceEditorController:
    def __init__(
        ...
        # # Connect datafile changes to column updates
        # datafile_combo = view.widgets.get("datafile")
        # if datafile_combo:
        #     # Disconnect any existing connections first to avoid duplicates
        #     try:
        #         datafile_combo.currentTextChanged.disconnect()
    ...
    # def on_datafile_changed(self, display_str: Union[str, DataFileMetadata]):
    #     """Handle datafile changes using display strings or metadata objects."""
    #     # Skip empty strings
    #     if isinstance(display_str, str) and not display_str.strip():
    #         return
            
    #     # Get the metadata from the display string or DataFileMetadata object
    #     if isinstance(display_str, str):
    #         # First try getting it by display string
    #         metadata = self.data_library.get_metadata_by_display_string(display_str)
            
    #         if metadata is None:
    #             # If not found, try by file path
    #             metadata = self.data_library.get_metadata_by_path(display_str)
                
    #             if metadata is None:
    #                 # As a last resort, create a new metadata object
    #                 print(f"Warning: No metadata found for '{display_str}', creating new")
    #                 try:
    #                     metadata = DataFileMetadata.from_display_string(display_str)
    #                 except Exception as e:
    #                     print(f"Error parsing display string: {e}")
    #                     # Just use the string as file path with default values
    #                     metadata = DataFileMetadata(file_path=display_str)
    #     elif isinstance(display_str, DataFileMetadata):
    #         metadata = display_str
    #     else:
    #         print(f"Unexpected datafile type: {type(display_str)}")
    #         return

    #     # Update the model's datafile
    #     self.model.datafile = metadata

    #     # Get the dataframe and update column options
    ...
    #     self.update_filter_columns_for_datafile(all_cols)
