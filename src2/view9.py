...
class TraceEditorModel:
    ...
    # Filters is kept as a simple list here.
    filters: list = field(
        default_factory=list, 
        metadata={
            "label": "Filters:",  # Not rendered; could be a complex sub-view.
            "widget": None,
            "plot_types": ["ternary", "cartesian"]
        }
    )
...
class TraceEditorView(QWidget):
    """
    A view that automatically builds its UI based on the metadata defined in TraceEditorModel.
    It shows only the fields that are applicable for the current plot type.
    When switching between traces, the UI is preserved and updated with the new model's values.
    """
    def __init__(self, model, parent=None):
        super().__init__(parent)
        self.model = model  # A TraceEditorModel instance.
        self.current_plot_type = "ternary"  # Default plot type.
        self.widgets = {}  # Mapping of field names to widget instances.
    ...
    def _build_ui(self):
        # Build the form only once using the model's dataclass fields.
        for f in fields(self.model):
            ...
            # Initialize the widget from the model's current value.
            ...
            if isinstance(widget, QLineEdit):
                ...
                # Example for line_style; customize for other combo fields as needed.
    ...
    def set_plot_type(self, plot_type: str):
        """
        Adjusts the visibility of form fields based on the given plot type.
        Rebuild the UI only when the plot type changes.
        """
    ...
    def update_from_model(self):
        """
        Refresh widget values from the model.
        Useful when the model has been updated externally.
        """
        for f in fields(self.model):
            metadata = f.metadata
    ...
    def set_model(self, new_model):
        """
        Switches to a new TraceEditorModel (i.e. a different trace).
        The UI widgets remain intact; only their values are updated.
        """
